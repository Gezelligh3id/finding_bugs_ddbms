...
static std::regex e_timeout("ERROR:  canceling statement due to statement timeout(\n|.)*");
static std::regex e_syntax("ERROR:  syntax error at or near(\n|.)*");
...
void processQuery(std::string& query) {
    // 定义用于匹配以SELECT开头的正则表达式
    std::regex select_regex(R"(^\s*SELECT)", std::regex_constants::icase);

    // 如果匹配成功，则在前面加上";\n"
    if (std::regex_search(query, select_regex)) {
        query = ";\n" + query;
    }
}
...
void log_query(const std::string& original_query) {
    ...
    log_file.open("query.log", std::ios_base::app); // 以追加模式打开文件
    std::string query = original_query; 
    std::regex select_regex(R"(^\s*SELECT\s+(unnest\(\s*proargtypes\s*\)|table_name|attname|conname|typname|oprname))", std::regex_constants::icase);
    if (std::regex_search(query, select_regex)) {
        return;
    }
    ...
    : pgsql_connection(db, port)
{
    ...
    auto res = PQexec(conn, set_timeout_cmd.c_str());
    if (is_start) {
        log_query(set_timeout_cmd);
    }
    set_timeout_cmd = "SET citus.shard_replication_factor TO 1; SET citus.enable_repartition_joins to ON;";
    res = PQexec(conn, set_timeout_cmd.c_str());
    if (is_start) {
        log_query(set_timeout_cmd);
        is_start = false;
    }
