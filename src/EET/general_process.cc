...
void execute_sql(dbms_info& d_info, const std::string& sql) {
    try {
        auto dut = dut_setup(d_info);
        dut->test(sql, nullptr, nullptr);
...
{ 
    ...
    // router_planner
        ...
        ddl_stmt_num = 3;
    // query_pushdown_planner
    else if (rounds % 4 == 2)
        ddl_stmt_num = 3;
    // query_join_order_planner
    else
        ddl_stmt_num = 3;
    ...
    // fast_path planner
    // create 1 local 1 distributed 1 reference
    if (rounds % 4 == 0) {
        execute_sql(d_info, "SELECT create_distributed_table('t1', 'vkey');");
        execute_sql(d_info, "SELECT create_reference_table('t2');");
    }
        
    // router_planner
    // create 2 distributed 1 reference
    else if (rounds % 4 == 1) {
        execute_sql(d_info, "SELECT create_distributed_table('t0', 'vkey');");
        execute_sql(d_info, "SELECT create_distributed_table('t1', 'vkey');");
        execute_sql(d_info, "SELECT create_reference_table('t2');");
    }
    ...
    // query_join_order_planner
    // create 2 distributed
    else {
        execute_sql(d_info, "SELECT create_distributed_table('t0', 'pkey');");
        execute_sql(d_info, "SELECT create_distributed_table('t1', 'vkey');");
        execute_sql(d_info, "SELECT create_reference_table('t2');");
    }
