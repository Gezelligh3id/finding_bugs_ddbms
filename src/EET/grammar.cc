...
shared_ptr<table_ref> table_ref::factory(prod *p, bool no_join) {
    try {
        if (p->level < 3 + d6()) {
            // fast_path
            if (rounds % 4 == 0) {
                make_shared<table_or_query_name>(p);
            }
            if (d6() > 5 && p->level < d6())
...
{
    out << "DROP TABLE IF EXISTS " << created_table->name << ";\n";
    out << "create table ";
    ...
    // out << ";SELECT create_distributed_table('" << created_table->name << "', 'vkey');";
}
...
{
    // generate_join_query(s);
    cerr << "statement_factory" << endl;
    try {
        s->new_stmt();
...
// void generate_join_query(scope *s) {
//     auto p = make_shared<prod>();  // 创建一个生产者对象

//     // 创建一个 query_spec 对象，它将生成一个包含 JOIN 的 SELECT 语句
//     auto query = make_shared<query_spec>(p.get(), s);

//     // 创建 joined_table 对象来生成 JOIN 操作
//     auto join_table = make_shared<joined_table>(p.get());

//     // 创建一个 from_clause 对象，将 JOIN 表加入到 FROM 子句中
//     auto from = make_shared<from_clause>(p.get(), false);
//     from->reflist.push_back(join_table);

//     // 将 from_clause 和 joined_table 添加到 query_spec 对象中
...
// void generate_simple_select_query(scope *s) {
//     auto query = make_shared<query_spec>(nullptr, s);
//     cout << *query << endl;
// }